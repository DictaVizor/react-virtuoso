...
export function initialSizeState() {
    return {
        offsetTree: [],
        sizeTree: newTree(),
...
export function indexComparator({ index: itemIndex }, index) {
    return index === itemIndex ? 0 : index < itemIndex ? -1 : 1;
}
export function offsetComparator({ offset: itemOffset }, offset) {
    return offset === itemOffset ? 0 : offset < itemOffset ? -1 : 1;
}
function offsetPointParser(point) {
    return { index: point.index, value: point };
}
export function rangesWithinOffsets(tree, startOffset, endOffset, minStartIndex = 0) {
    if (minStartIndex > 0) {
...
export function sizeStateReducer(state, [ranges, groupIndices, log]) {
    if (ranges.length > 0) {
...
export const sizeSystem = u.system(([{ log }]) => {
    ...
    const groupIndices = u.statefulStream([]);
    ...
    const sizes = u.statefulStreamFromEmitter(u.pipe(sizeRanges, u.withLatestFrom(groupIndices, log), u.scan(sizeStateReducer, initial), u.distinctUntilChanged()), initial);
    u.connect(u.pipe(groupIndices, u.filter((indexes) => indexes.length > 0), u.withLatestFrom(sizes), u.map(([groupIndices, sizes]) => {
    ...
    u.connect(u.pipe(totalCount, u.withLatestFrom(sizes), u.filter(([totalCount, { lastIndex }]) => {
    ...
    const trackItemSizes = u.statefulStreamFromEmitter(u.pipe(fixedItemSize, u.map((size) => size === undefined)), true);
    u.connect(u.pipe(defaultItemSize, u.filter((value) => {
        return value !== undefined && empty(u.getValue(sizes).sizeTree);
    }), u.map((size) => [{ startIndex: 0, endIndex: 0, size }])), sizeRanges);
    const listRefresh = u.streamFromEmitter(u.pipe(sizeRanges, u.withLatestFrom(sizes), u.scan(({ sizes: oldSizes }, [_, newSizes]) => {
        return {
            changed: newSizes !== oldSizes,
            sizes: newSizes,
        };
    }, { changed: false, sizes: initial }), u.map((value) => value.changed)));
    u.subscribe(u.pipe(firstItemIndex, u.scan((prev, next) => {
        return { diff: prev.prev - next, prev: next };
    }, { diff: 0, prev: 0 }), u.map((val) => val.diff)), (offset) => {
        if (offset > 0) {
    ...
    u.subscribe(u.pipe(firstItemIndex, u.withLatestFrom(log)), ([index, log]) => {
        if (index < 0) {
            log("`firstItemIndex` prop should not be set to less than zero. If you don't know the total count, just use a very high value", { firstItemIndex }, LogLevel.ERROR);
        }
    });
    // Capture the current list top item before the sizes get refreshed
    const beforeUnshiftWith = u.streamFromEmitter(unshiftWith);
    u.connect(u.pipe(unshiftWith, u.withLatestFrom(sizes), u.map(([unshiftWith, sizes]) => {
        if (sizes.groupIndices.length > 0) {
            throw new Error('Virtuoso: prepending items does not work with groups');
        }
        return walk(sizes.sizeTree).reduce((acc, { k: index, v: size }) => {
            return {
    ...
    const shiftWithOffset = u.streamFromEmitter(u.pipe(shiftWith, u.withLatestFrom(sizes), u.map(([shiftWith, { offsetTree }]) => {
        const newFirstItemIndex = -shiftWith;
        return offsetOf(newFirstItemIndex, offsetTree);
    })));
    u.connect(u.pipe(shiftWith, u.withLatestFrom(sizes), u.map(([shiftWith, sizes]) => {
        if (sizes.groupIndices.length > 0) {
...
//# sourceMappingURL=sizeSystem.js.map