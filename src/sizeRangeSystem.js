...
export const sizeRangeSystem = u.system(([{ scrollTop, viewportHeight, deviation, headerHeight }]) => {
    ...
    const visibleRange = u.statefulStreamFromEmitter(u.pipe(u.combineLatest(u.duc(scrollTop), u.duc(viewportHeight), u.duc(headerHeight), u.duc(listBoundary, tupleComparator), u.duc(overscan), u.duc(topListHeight), u.duc(fixedHeaderHeight), u.duc(deviation), u.duc(increaseViewportBy), u.duc(extraSize)), u.map(([scrollTop, viewportHeight, headerHeight, [listTop, listBottom], overscan, topListHeight, fixedHeaderHeight, deviation, increaseViewportBy, extraSize,]) => {
        ...
        return null;
    }), u.filter((value) => value != null), u.distinctUntilChanged(tupleComparator)), [0, 0]);
    return {
        // input
        listBoundary,
        overscan,
...
}, u.tup(domIOSystem), { singleton: true });
