...
export const initialItemCountSystem = u.system(([{ sizes, firstItemIndex, data }, { listState }, { didMount }]) => {
    const initialItemCount = u.statefulStream(0);
    u.connect(u.pipe(didMount, u.withLatestFrom(initialItemCount), u.filter(([, count]) => count !== 0), u.withLatestFrom(sizes, firstItemIndex, data), u.map(([[, count], sizes, firstItemIndex, data = []]) => {
...
//# sourceMappingURL=initialItemCountSystem.js.map