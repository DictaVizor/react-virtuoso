...
export function buildListState(items, topItems, totalCount, sizes, firstItemIndex) {
    ...
    const extraSize = filteredItems.reduce((a, b) => a + b.size, 0);
    if (items.length > 0) {
...
export const listStateSystem = u.system(([{ sizes, totalCount, data, firstItemIndex }, groupedListSystem, { visibleRange, listBoundary, topListHeight: rangeTopListHeight, customStartIndex, extraSize, keepIndexRendered }, { scrolledToInitialItem, initialTopMostItemIndex }, { topListHeight }, stateFlags, { didMount },]) => {
    const topItemsIndexes = u.statefulStream([]);
    const itemsRendered = u.stream();
    const startIndex = u.statefulStream(undefined);
    u.connect(groupedListSystem.topItemsIndexes, topItemsIndexes);
    u.connect(customStartIndex, startIndex);
    const listState = u.statefulStreamFromEmitter(u.pipe(u.combineLatest(didMount, u.duc(visibleRange), u.duc(totalCount), u.duc(sizes), u.duc(initialTopMostItemIndex), scrolledToInitialItem, u.duc(topItemsIndexes), u.duc(firstItemIndex), startIndex, keepIndexRendered, data), u.filter(([mount]) => mount), u.map(([, [startOffset, endOffset], totalCount, sizes, initialTopMostItemIndex, scrolledToInitialItem, topItemsIndexes, firstItemIndex, customStartIndex, keepIndexRendered, data,]) => {
        ...
        if (customStartIndex !== undefined) {
            if (customStartIndex < firstIndex) {
                const customOffsetPointRanges = rangesWithinOffsets(offsetTree, 0, startOffset, minStartIndex).filter(({ start }) => start >= customStartIndex && start < firstIndex);
                ...
                const customOffsetPointRanges = rangesWithinOffsets(offsetTree, startOffset, Infinity, minStartIndex).filter(({ start }) => start <= customStartIndex && lastIndex <= start);
                offsetPointRanges = uniqBy([
                    ...customOffsetPointRanges.map((range) => {
                        return { ...range, isCustom: true };
                    }),
        ...
        const extraSize = offsetPointRanges.filter((range) => range.isCustom).reduce((a, b) => a + b.value.size, 0);
        const items = u.tap([], (result) => {
    ...
    u.filter((value) => value !== null), u.distinctUntilChanged()), EMPTY_LIST_STATE);
    u.connect(u.pipe(data, u.filter((data) => data !== undefined), u.map((data) => data.length)), totalCount);
    u.connect(u.pipe(listState, u.map(u.prop('topListHeight'))), topListHeight);
    u.connect(topListHeight, rangeTopListHeight);
    u.connect(u.pipe(listState, u.map((state) => [state.top, state.bottom])), listBoundary);
    u.connect(u.pipe(listState, u.map((state) => state.extraSize)), extraSize);
    u.connect(u.pipe(listState, u.map((state) => state.items)), itemsRendered);
    const endReached = u.streamFromEmitter(u.pipe(listState, u.filter(({ items }) => items.length > 0), u.withLatestFrom(totalCount, data), u.filter(([{ items }, totalCount]) => items[items.length - 1].originalIndex === totalCount - 1), u.map(([, totalCount, data]) => [totalCount - 1, data]), u.distinctUntilChanged(tupleComparator), u.map(([count]) => count)));
    const startReached = u.streamFromEmitter(u.pipe(listState, u.throttleTime(200), u.filter(({ items, topItems }) => {
        return items.length > 0 && items[0].originalIndex === topItems.length;
    }), u.map(({ items }) => items[0].index), u.distinctUntilChanged()));
    const rangeChanged = u.streamFromEmitter(u.pipe(listState, u.filter(({ items }) => items.length > 0), u.map(({ items }) => {
        return {
...
//# sourceMappingURL=listStateSystem.js.map