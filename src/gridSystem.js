...
function buildItems(startIndex, endIndex) {
    return Array.from({ length: endIndex - startIndex + 1 }).map((_, i) => ({ index: i + startIndex }));
}
export const gridSystem = u.system(([{ overscan, visibleRange, listBoundary }, { scrollTop, viewportHeight, scrollBy, scrollTo, smoothScrollTargetReached, scrollContainerState }, stateFlags, scrollSeek, { propsReady, didMount }, { windowViewportRect, windowScrollTo, useWindowScroll, customScrollParent, windowScrollContainerState },]) => {
    ...
    u.connect(u.pipe(didMount, u.withLatestFrom(initialItemCount), u.filter(([, count]) => count !== 0), u.map(([, count]) => {
        return {
    ...
    u.connect(u.pipe(u.combineLatest(u.duc(totalCount), visibleRange, u.duc(itemDimensions, (prev, next) => prev && prev.width === next.width && prev.height === next.height)), u.withLatestFrom(viewportDimensions), u.map(([[totalCount, [startOffset, endOffset], item], viewport]) => {
    ...
    u.connect(u.pipe(viewportDimensions, u.map(({ height }) => height)), viewportHeight);
    u.connect(u.pipe(u.combineLatest(viewportDimensions, itemDimensions, gridState), u.map(([viewportDimensions, item, { items }]) => {
        const { top, bottom } = gridLayout(viewportDimensions, item, items);
        return [top, bottom];
    }), u.distinctUntilChanged(tupleComparator)), listBoundary);
    const endReached = u.streamFromEmitter(u.pipe(u.duc(gridState), u.filter(({ items }) => items.length > 0), u.withLatestFrom(totalCount), u.filter(([{ items }, totalCount]) => items[items.length - 1].index === totalCount - 1), u.map(([, totalCount]) => totalCount - 1), u.distinctUntilChanged()));
    const startReached = u.streamFromEmitter(u.pipe(u.duc(gridState), u.filter(({ items }) => {
        return items.length > 0 && items[0].index === 0;
    }), u.mapTo(0), u.distinctUntilChanged()));
    const rangeChanged = u.streamFromEmitter(u.pipe(u.duc(gridState), u.filter(({ items }) => items.length > 0), u.map(({ items }) => {
        return {
    ...
    u.connect(u.pipe(scrollToIndex, u.withLatestFrom(viewportDimensions, itemDimensions, totalCount), u.map(([location, viewport, item, totalCount]) => {
    ...
    const totalListHeight = u.statefulStreamFromEmitter(u.pipe(gridState, u.map((gridState) => {
        return gridState.offsetBottom + gridState.bottom;
    })), 0);
    u.connect(u.pipe(windowViewportRect, u.map((viewportInfo) => ({ width: viewportInfo.visibleWidth, height: viewportInfo.visibleHeight }))), viewportDimensions);
    return {
        // input
        totalCount,
        viewportDimensions,
