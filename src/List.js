...
import { compose, connect, getValue, map, pipe, publish, statefulStream, stream, subscribe, system, tup, withLatestFrom, statefulStreamFromEmitter, distinctUntilChanged, noop, } from '@virtuoso.dev/urx';
...
const combinedSystem = system(([listSystem, propsSystem]) => {
    ...
    function deprecateComponentProp(stream, componentName, propName) {
        connect(pipe(stream, withLatestFrom(propsSystem.components), map(([comp, components]) => {
...
export const Items = React.memo(function VirtuosoItems({ showTopList = false }) {
    ...
    const { callbackRef, ref } = useChangedListContentsSizes(sizeRanges, itemSize, trackItemSizes, showTopList ? noop : scrollContainerStateCallback, log, customScrollParent);
    ...
    return (React.createElement(React.Fragment, null, createElement(ListComponent, {
...
export function buildScroller({ usePublisher, useEmitter, useEmitterValue }) {
    const Scroller = React.memo(function VirtuosoScroller({ style, children, ...props }) {
        ...
        return createElement(ScrollerComponent, {
            ref: scrollerRef,
...
export const { Component: List, usePublisher, useEmitterValue, useEmitter, } = systemToComponent(combinedSystem, {
    required: {},
    optional: {
        context: 'context',
