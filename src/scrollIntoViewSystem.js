...
export const scrollIntoViewSystem = u.system(([{ sizes, totalCount }, { scrollTop, viewportHeight, headerHeight, scrollingInProgress }, { scrollToIndex }]) => {
    const scrollIntoView = u.stream();
    u.connect(u.pipe(scrollIntoView, u.withLatestFrom(sizes, viewportHeight, totalCount, headerHeight, scrollTop), u.map(([{ index, behavior = 'auto', done }, sizes, viewportHeight, totalCount, headerHeight, scrollTop]) => {
        ...
        if (location) {
                ...
                u.handleNext(u.pipe(scrollingInProgress, u.skip(1), u.filter((value) => value === false)), done);
        }
        else {
            done && done();
        }
        return location;
    }), u.filter((value) => value !== null)), scrollToIndex);
    return {
        scrollIntoView,
    };
}, u.tup(sizeSystem, domIOSystem, scrollToIndexSystem, listStateSystem, loggerSystem), { singleton: true });
//# sourceMappingURL=scrollIntoViewSystem.js.map